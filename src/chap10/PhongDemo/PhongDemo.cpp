...
class PhongDemo : public D3DApp
{
public:
	PhongDemo(HINSTANCE hInstance, std::wstring winCaption);
	virtual ~PhongDemo();
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE prevInstance, PSTR cmdLine, int showCmd)
{
	...
	PhongDemo app(hInstance, L"Phong Demo");
	gd3dApp = &app;
...
PhongDemo::PhongDemo(HINSTANCE hInstance, std::wstring winCaption)
	: D3DApp(hInstance, winCaption)
{
	...
	mSpecularMtrl  = D3DXCOLOR(0.6f, 0.6f, 0.6f, 1.0f);
...
void PhongDemo::onLostDevice()
{
	mGfxStats->onLostDevice();
	HR(mFX->OnLostDevice());
}

void PhongDemo::onResetDevice()
{
...
void PhongDemo::updateScene(float dt)
{
...
void PhongDemo::buildFX()
{
	ID3DXBuffer *errors = 0;
	HR(D3DXCreateEffectFromFile(gd3dDevice, L"../src/chap10/PhongDemo/phong.fx",
		0, 0, D3DXSHADER_DEBUG, 0, &mFX, &errors));
	if (errors)
		MessageBoxA(0, (char*)errors->GetBufferPointer(), 0, 0);

	mhTech                  = mFX->GetTechniqueByName("PhongTech");
