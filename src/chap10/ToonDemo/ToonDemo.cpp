...
class ToonDemo : public D3DApp
{
public:
	ToonDemo(HINSTANCE hInstance, std::wstring winCaption);
	virtual ~ToonDemo();
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE prevInstance, PSTR cmdLine, int showCmd)
{
	...
	ToonDemo app(hInstance, L"Toon Demo");
	gd3dApp = &app;
...
ToonDemo::ToonDemo(HINSTANCE hInstance, std::wstring winCaption)
	: D3DApp(hInstance, winCaption)
{
	...
	mLightVecW    = D3DXVECTOR3(-4.0f, 0.0f, -1.0f);
...
void ToonDemo::onLostDevice()
{
	mGfxStats->onLostDevice();
	HR(mFX->OnLostDevice());
}

void ToonDemo::onResetDevice()
{
...
void ToonDemo::updateScene(float dt)
{
...
void ToonDemo::buildFX()
{
	ID3DXBuffer *errors = 0;
	HR(D3DXCreateEffectFromFile(gd3dDevice, L"../src/chap10/ToonDemo/toon.fx",
		0, 0, D3DXSHADER_DEBUG, 0, &mFX, &errors));
	if (errors)
		MessageBoxA(0, (char*)errors->GetBufferPointer(), 0, 0);

	mhTech                  = mFX->GetTechniqueByName("ToonTech");
