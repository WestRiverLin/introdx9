...
{
public:
	GateDemo(HINSTANCE hInstance, std::wstring winCaption);
	~GateDemo();
				   ...
				   PSTR cmdLine, int showCmd)
{
	...
	GateDemo app(hInstance, L"Gate Demo");
	gd3dApp = &app;
...
GateDemo::GateDemo(HINSTANCE hInstance, std::wstring winCaption)
: D3DApp(hInstance, winCaption)
{
	...
	mGfxStats->addVertices(4);  // add gate quad vertices
...
GateDemo::~GateDemo()
{
	...
	SafeRelease(mGridVB);
	SafeRelease(mGridIB);
	SafeRelease(mGroundTex);
	SafeRelease(mGateVB);
	SafeRelease(mGateIB);
	SafeRelease(mGateTex);
	SafeRelease(mFX);
